# Hi, I'm Ben! 👋

I’m a Data Scientist building and supporting production ML and LLM systems for clinical and operational decision-making. 

I'm always looking for opportunities to learn or deepen my understanding of existing skills. I learn best by doing (checkout my most recent project!)  
🚀 [transcript-to-chat](https://github.com/Ben-Nolan/transcript-to-chat/blob/main/README.md) — *An interactive RAG sandbox that allows a user to visuzlize the RAG pipeline*

---

### 🛠 Tech Stack
- **Languages & Data:** Python, SQL, Snowflake
- **Machine Learning & AI:** scikit-learn, XGBoost, PyTorch, Hugging Face, LangChain
- **Infrastructure:** AWS, Docker, Kubernetes

---

### 🤓 Recent Book Shelf

*   ✅ **Completed:** *Practical MLOps* (Noah Gift & Alfredo Deza) & *Software Engineering for Data Scientists* (Catherine Nelson)
*   📖 **Currently Reading:** *AI Engineering* (Chip Huyen)
*   📚 **Future Reads:** *Designing Machine Learning Systems* (Chip Huyen)

*Got a recommendation? I'd love to hear it!*

---

### Let's chat! 
Find me on [LinkedIn](https://www.linkedin.com/in/ben-clark-nolan/) or shoot me an [Email](mailto:bcnolan34@gmail.com).
